...
job_filter <- function(df,input_vec) {
	...
	# A new column PW_LEVEL_9089 is created to identify the Job Type
	# If multiple job type inputs match with a single row in the dataframe df, the
	# output contains them in different rows each with distinct PW_LEVEL_9089

	# If input_vec is empty, return without any filtering
	if(length(input_vec) == 0) {
		return(df %>%
				 	dplyr::mutate(PW_LEVEL_9089 = JOB_INFO_JOB_TITLE))
	}
	...
	for(value in input_vec){
		new_df <- rbind(new_df, df %>%
							 	dplyr::filter(regexpr(value,JOB_INFO_JOB_TITLE,ignore.case=TRUE) != -1) %>%
							 	dplyr::mutate(PW_LEVEL_9089 = toupper(value)))
	}
...
find_top <- function(df,x_feature,metric, Ntop = 3) {
	...
	df %>%
		dplyr::group_by_(x_feature) %>%
		dplyr::mutate(certified =ifelse(CASE_STATUS == "CERTIFIED",1,0)) %>%
		dplyr::summarise(TotalApps = n(),
					 Time_med_d = median(TIMEd), # instead of PREVAILING WAGE
					 #Wage = median(PREVAILING_WAGE),
					 CertiApps = sum(certified),
					 Share = CertiApps/850) %>%
		dplyr::arrange_(arrange_criteria) -> top_df
...
plot_input <- function(df, x_feature, fill_feature, metric,filter = FALSE, ...) {
	...
	return(df %>%
			 	dplyr::group_by_(.dots=c(x_feature,fill_feature)) %>%
			 	dplyr::mutate(certified =ifelse(CASE_STATUS == "CERTIFIED",1,0)) %>%
			 	dplyr::summarise(TotalApps = n(),
			 				 CertiApps = sum(certified),
			 				 Time_med_d = median(TIMEd), # instead of PREVAILING WAGE
...
map_gen <- function(df,metric,USA,...) {
	...
	df %>%
		dplyr::mutate(certified =ifelse(CASE_STATUS == "CERTIFIED",1,0)) %>%
		dplyr::group_by(WORKSITE,lat,lon) %>%
		dplyr::summarise(TotalApps = n(),CertiApps = sum(certified), Wage = median(PREVAILING_WAGE)) -> map_df
